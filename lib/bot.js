...
class Bot {
  ...
  attachListeners() {
    ...
    this.ircClient.on('names', (channel, nicks) => {
      ...
      if (nickList.length) {
        const text = `Na IRC *${channel}* visijo: **${nickList.join(', ')}**`;
        this.sendToDiscord(this.nickname, channel, text);
      }
    });

    this.ircClient.on('topic', (channel, topic, nick, message) => {
      const text = `*${ channel } IRC topic:*  ${ stripColorsAndStyle(topic) }`;
      this.sendToDiscord(this.nickname, channel, text);
    });
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {        
        ...
        this.parseCommand(text, ircChannel);
